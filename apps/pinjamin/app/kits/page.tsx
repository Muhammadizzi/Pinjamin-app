      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-muted-foreground">Bundel aset yang dipinjam sebagai paket • {kits.length} kit</p>
          </div>
          <Link href="/kits/new">
        ...
        {kits.length === 0 ? (
          <Card className="border-dashed">
            <CardContent className="py-16 text-center space-y-3">
              ...
              <div>
                ...
                <div className="text-sm text-muted-foreground">Kit adalah bundel aset yang dipinjam bersamaan</div>
              </div>
              <Link href="/kits/new">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
            {kits.map((k) => {
              const cat = categories.find((c) => c.id === (k as any).categoryId);
              const loc = locations.find((l) => l.id === (k as any).locationId);
              return (
                <Card key={k.id} className="hover:shadow-lg transition-all hover:-translate-y-1 overflow-hidden group">
                  {(k as any).image ? (
                    <div className="h-36 w-full overflow-hidden bg-slate-100">
                      <img src={(k as any).image} alt={k.name} className="h-full w-full object-cover group-hover:scale-105 transition-transform duration-300" />
                    </div>
                  ) : (
                  ...
                  <CardHeader className="pb-2">
                    ...
                    <p className="text-xs text-muted-foreground line-clamp-2 min-h-[32px]">{k.description || "-"}</p>
                  </CardHeader>
                  <CardContent className="space-y-3">
                    ...
                    <div className="space-y-1 max-h-24 overflow-auto pr-1">
                      {k.assetIds.slice(0, 3).map((aid) => {
                        ...
                        return a ? (
                          <div key={aid} className="text-xs flex items-center gap-2 bg-slate-50 dark:bg-slate-800 rounded-lg px-2 py-1.5">
                            <Package className="h-3 w-3 shrink-0 text-slate-500" />
                            <span className="truncate">{a.name}</span>
                          </div>
                        ) : null;
                      })}
                      {k.assetIds.length > 3 && <div className="text-xs text-muted-foreground text-center">+{k.assetIds.length - 3} aset lagi</div>}
                    </div>
                    <div className="flex gap-2 pt-1">
                      ...
                      <Button variant="ghost" size="sm" onClick={() => deleteKit(k.id)} className="text-red-600 hover:text-red-700 hover:bg-red-50">
